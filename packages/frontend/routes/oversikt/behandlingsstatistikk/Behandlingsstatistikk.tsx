...
import styles from './Behandlingsstatistikk.module.css';
...
const AutomatiskUtbetaltePerioderIkon = () => {
    ...
    return (
        <Tooltip content="Perioder som er automatisk utbetalt">
            <img
                src={imageData}
...
const UtbetaltePerioderIkon = () => {
    ...
    return (
        <Tooltip content="Perioder som er manuelt utbetalt">
            <img src={imageData} aria-label="Perioder som er manuelt utbetalt" alt="Perioder som er manuelt utbetalt" />
        </Tooltip>
    );
};
...
const AnnullertePerioderIkon = () => {
    ...
    return (
        <Tooltip content="Perioder som er annullert">
            <img src={imageData} aria-label="Perioder som er annullert" alt="Perioder som er annullert" />
        </Tooltip>
    );
};

const mapStatistikk = (eksternStatistikk: ExternalBehandlingstatistikk): ExternalBehandlingstatistikk => ({
    fullførteBehandlinger: {
        ...eksternStatistikk.fullførteBehandlinger,
        manuelt: {
            ...eksternStatistikk.fullførteBehandlinger.manuelt,
            perPeriodetype: eksternStatistikk.fullførteBehandlinger.manuelt.perPeriodetype.sort((a, b) =>
                a.periodetypeForSpeil.localeCompare(b.periodetypeForSpeil),
            ),
        },
    },
    antallOppgaverTilGodkjenning: {
        ...eksternStatistikk.antallOppgaverTilGodkjenning,
        perPeriodetype: eksternStatistikk.antallOppgaverTilGodkjenning.perPeriodetype.sort((a, b) =>
            a.periodetypeForSpeil.localeCompare(b.periodetypeForSpeil),
        ),
    },
    antallTildelteOppgaver: {
        ...eksternStatistikk.antallTildelteOppgaver,
        perPeriodetype: eksternStatistikk.antallTildelteOppgaver.perPeriodetype.sort((a, b) =>
            a.periodetypeForSpeil.localeCompare(b.periodetypeForSpeil),
        ),
    },
});

const behandlingsstatistikkState = selector<ExternalBehandlingstatistikk>({
    key: 'behandlingsstatistikkState',
    get: async () => {
        return await getBehandlingsstatistikk()
            .then((res) => mapStatistikk(res))
            .catch(() => {
...
const getDataTipForPeriodetype = (periodetype: ExternalPeriodetype): string => {
    switch (periodetype) {
        case 'FORLENGELSE':
            return 'Forlengelser';
...
export const Behandlingsstatistikk = () => {
    ...
    const statistikk =
        loadableStatistikk.state === 'hasValue'
            ? (loadableStatistikk.contents as ExternalBehandlingstatistikk)
            : undefined;

...
export const useUvurderteVarslerPåPeriode = (periode: FetchedBeregnetPeriode | DatePeriod): boolean => {
    ...
    if (
        (!isBeregnetPeriode(periode) && !isUberegnetPeriode(periode) && !isUberegnetVilkarsprovdPeriode(periode)) ||
        !arbeidsgiver
    ) {
        return false;
    }
...
export const useHarUvurderteVarslerPåEllerFør = (
    ...
    return arbeidsgivere
        ...
        .some((periode) => {
            if (!isBeregnetPeriode(periode) && !isUberegnetPeriode(periode) && !isUberegnetVilkarsprovdPeriode(periode))
                return false;
            return periode.varsler
                .filter((varsel) => !varsel.kode.startsWith('SB_BO_'))
                .some(

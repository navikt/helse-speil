...
describe("isValidIn", () => {
  ...
  const tokenThatExpiresIn = (seconds: number) =>
    createToken({ exp: `${nowInSeconds + seconds}` });

  test("time to check is before exp", () => {
    expect(
      auth.isValidIn({ seconds: 28, token: tokenThatExpiresIn(30) })
    ).toEqual(true);
  });

  test("time to check is at exp", () => {
    expect(
      auth.isValidIn({ seconds: 30, token: tokenThatExpiresIn(30) })
    ).toEqual(false);
  });

  test("time to check is after exp", () => {
    expect(
      auth.isValidIn({ seconds: 31, token: tokenThatExpiresIn(30) })
    ).toEqual(false);
  });

  test("the exp has already passed", () => {
    expect(
      auth.isValidIn({ seconds: 1, token: tokenThatExpiresIn(-30) })
    ).toEqual(false);
  });

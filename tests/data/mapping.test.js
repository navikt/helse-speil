...
test('medlemsskap', async () => {
   ...
   const medlemsskap = mapping.inngangsvilkår(rawServerResponse.behandlinger[0]).medlemsskap
   const expected = {
      bostedsland: 'NOR'
   }
...
test('opptjening', async () => {
   const rawServerResponse = JSON.parse(readTestdata())
   const opptjening = mapping.inngangsvilkår(rawServerResponse.behandlinger[0]).opptjening
   const expected = {
...
test('inntekt', async () => {
   const rawServerResponse = JSON.parse(readTestdata())
   const mapped = mapping.inngangsvilkår(rawServerResponse.behandlinger[0]).inntekt
   const expected = {
      beløp: 300000
   }
...
test('søknadsfrist', async () => {
   const rawServerResponse = JSON.parse(readTestdata())
   const mapped = mapping.inngangsvilkår(rawServerResponse.behandlinger[0]).søknadsfrist
   const expected = {
      sendtNav: new Date('2019-03-14T08:17:55.459Z'),
...
test('dagerIgjen', async () => {
   const rawServerResponse = JSON.parse(readTestdata())
   const mapped = mapping.inngangsvilkår(rawServerResponse.behandlinger[0]).dagerIgjen
   const expected = {

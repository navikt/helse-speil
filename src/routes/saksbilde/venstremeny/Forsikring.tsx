...
export const Forsikring = ({
    ...
    return (
        <>
            <BodyShort>Dekning</BodyShort>
            <BodyShort>
                {erUtvikling && data?.eksisterer
                    ? `${data.forsikringInnhold?.dekningsgrad.toString()} % fra ${data.forsikringInnhold?.gjelderFraDag.valueOf()}. dag`
                    : `${erJordbruker ? 100 : 80} % fra 17. dag`}
            </BodyShort>
        </>
    );
};

...
export const videresendTilVilkarsproving = async (request: Request): Promise<Response> => {
    const headersResult = await buildHeaders(request);
    if (headersResult instanceof Response) {
        return headersResult;
    }

    return fetch(vilkarsprovingUrl(request), buildBody(request, headersResult));
};
...
const vilkarsprovingUrl = (request: Request) => {
    const vilkarsprovingRelativeUrl = `/api/${substringAfter(request.url, '/api/vilkarsproving/')}`;
    return getServerEnv().VILKARSPROVING_BASEURL + vilkarsprovingRelativeUrl;
};
